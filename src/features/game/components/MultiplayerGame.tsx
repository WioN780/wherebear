...
import { RoomConfig, GameMode, Difficulty } from "../../multiplayer/types";
import {
  Compass,
  ArrowLeft,
  Globe,
...
export const MultiplayerGame: React.FC<MultiplayerGameProps> = ({
  ...
  // Reset local guess and searching state on room change
  ...
  const handleQuickMatch = () => {
    ...
    // Reset searching if no room after 10s (timeout/error)
    setTimeout(() => {
      setIsSearching(false);
    }, 10000);
  };

  // Handle local guess placement on picker map
                  ...
                  friendList.map((friend) => (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-6 h-6 rounded-full bg-white/5 border border-white/10 flex items-center justify-center font-bold text-[10px] uppercase text-zinc-300">
                          {friend.image ? (
                            <img
                              src={friend.image}
                              alt={friend.username}
  ...
  // Find user's player record in room
  ...
  // 5. ROUND REVIEW STATE (RoundReview)
  if (room.state === "RoundReview") {
    // Sort players by points gained in this round descending
    const sortedPlayers = [...room.players].sort(
      (a, b) => (b.lastScore || 0) - (a.lastScore || 0),
    );

    return (
      <div className="absolute inset-0 bg-zinc-950 select-none">
        {/* Full-width Map Background for results */}
        <div className="absolute inset-0 z-10">
          <MultiplayerMapPicker
            ...
            onSubmitGuess={() => {}}
            isSubmitted={true}
            className="w-full h-full border-none rounded-none top-0 left-0"
            currentUserId={currentUserId}
          />
        </div>
        ...
        {/* Left Floating Card - Round Scores Standings */}
        <div className="absolute top-24 left-6 bottom-6 z-20 w-[330px] pointer-events-none">
          <GlassCard className="p-6 border border-white/10 bg-zinc-950/70 backdrop-blur-xl pointer-events-auto flex flex-col gap-4 shadow-2xl h-full max-h-[85vh] overflow-y-auto">
            ...
            {/* Players Leaderboard this round */}
  ...
  // 6. MATCH GAME COMPLETE STATE (GameComplete)
            ...
            <div className="grid grid-cols-3 gap-4 max-w-xl mx-auto w-full items-end pt-8 pb-4">
              ...
              {podium[1] ? (
                <div className="flex flex-col items-center">
                  <div className="w-10 h-10 rounded-full border border-zinc-400 bg-zinc-800 text-white flex items-center justify-center font-bold text-xs uppercase mb-2">
                    {podium[1].image ? (
                      <img
                        src={podium[1].image}
                        alt=""
              ...
              {podium[0] && (
                <div className="flex flex-col items-center">
                  <div className="w-12 h-12 rounded-full border-2 border-yellow-500 bg-zinc-800 text-white flex items-center justify-center font-extrabold text-sm uppercase mb-2 ring-4 ring-yellow-500/20 shadow-[0_0_15px_rgba(234,179,8,0.3)]">
                    {podium[0].image ? (
                      <img
                        src={podium[0].image}
                        alt=""
              ...
              {podium[2] ? (
                <div className="flex flex-col items-center">
                  <div className="w-10 h-10 rounded-full border border-amber-700 bg-zinc-800 text-white flex items-center justify-center font-bold text-xs uppercase mb-2">
                    {podium[2].image ? (
                      <img
                        src={podium[2].image}
                        alt=""
              ...
              {rest.map((p, idx) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="w-6 h-6 rounded-full bg-zinc-800 border border-white/10 text-white flex items-center justify-center font-bold text-[10px] uppercase">
                      {p.image ? (
                        <img
                          src={p.image}
                          alt=""
