...
export const MultiplayerLobby: React.FC<MultiplayerLobbyProps> = ({
  ...
  const handleConfigChange = (updatedFields: Partial<RoomConfig>) => {
    if (!isHost) return;
    const newConfig: RoomConfig = { ...config, ...updatedFields };
    if (
          ...
          {config.mode !== "INFINITE" && (
            <div className="flex flex-col gap-2">
              <div className="flex justify-between text-xs font-bold text-zinc-400 uppercase tracking-wider">
                <span>Rounds</span>
                <span className="text-white font-mono">{config.totalRounds}</span>
              </div>
              {isHost ? (
                <input
                  type="range"
                  ...
                  onChange={(e) =>
                    handleConfigChange({ totalRounds: parseInt(e.target.value) })
                  }
